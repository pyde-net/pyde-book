...
|---------|---------|------|
| `otigen init <name> --lang <rust\|as\|go\|c>` | Scaffold a new project directory from the language template. Writes `otigen.toml` + a hello-world contract + language-specific build config (Cargo.toml / package.json + asconfig.json / go.mod / Makefile). The Rust scaffold uses the macro substrate (`#[pyde::entry]` + `pyde::declare_storage!()` + `pyde::declare_events!()`); the Go and C scaffolds get the same typed surface from generated code (`pyde_gen.go` / `pyde_gen.h`) that `otigen build` regenerates from `otigen.toml`; the AssemblyScript scaffold ships generated entry dispatch and typed storage accessors, with events over the raw host-fn pattern. | [§3.1](../companion/OTIGEN_BINARY_SPEC.md#31-otigen-init) |
| `otigen new <name> --from <template>` | Scaffold by cloning a canonical example bundle. Eleven templates ship today (`counter`, `token`, `nft`, `fungible-token`, `nft-token`, `simple-multisig`, `upgradeable-proxy`, `merkle-claim-airdrop`, `vesting`, `dao-governance`, `factory`), all on the `#[pyde::entry]` + `declare_storage!()` macro substrate, all building clean. `token` and `nft` are config-only PTS templates (`pts-f/1` and `pts-n/1`) generated from `otigen.toml`; `fungible-token` and `nft-token` are the editable reference sources for the same two standards. Produces a fully-working contract + passing test suite: the fastest path from zero to a green `otigen test`. `--list` shows the catalog. | [§3.11](../companion/OTIGEN_BINARY_SPEC.md#311-otigen-new) |
