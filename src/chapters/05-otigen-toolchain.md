...
|---------|---------|------|
| `otigen init <name> --lang <rust\|as\|go\|c>` | Scaffold a new project directory from the language template. Writes `otigen.toml` + a hello-world contract + language-specific build config (Cargo.toml / package.json + asconfig.json / go.mod / Makefile). The Rust scaffold uses the macro substrate (`#[pyde::entry]` + `pyde::declare_storage!()` + `pyde::declare_events!()`); the Go and C scaffolds get the same typed surface from generated code (`pyde_gen.go` / `pyde_gen.h`) that `otigen build` regenerates from `otigen.toml`; the AssemblyScript scaffold ships generated entry dispatch over the raw host-fn pattern. | [§3.1](../companion/OTIGEN_BINARY_SPEC.md#31-otigen-init) |
