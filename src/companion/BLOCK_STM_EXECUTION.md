...
| **L4: GPU acceleration for PQ crypto** | Move FALCON verify (per-tx, and per-reveal for the private mempool) off CPU. PQ signature verification is the per-tx tax that dominates execution at scale. | 5-10× on signature-heavy waves | v2: driver work |
