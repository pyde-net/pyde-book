...
| --- | --- |
| `otigen new` | Clone a canonical template (counter, erc20-token, erc721-token, …). Fastest path from zero to a green test run. |
...
| `otigen wallet` | Manage FALCON-512 keystore. New / import / export / sign / verify. |
| `otigen deploy` | Sign + submit a deploy transaction, poll for the receipt. |
| `otigen call` | Invoke a function on a deployed contract. View mode is free; `--from` switches to a state-mutating signed tx. |
| `otigen inspect` | Read contract / account metadata + state. Substrate `--state-field` for typed scalar reads. |
...
| `otigen devnet` | Run a local devnet embedded in the `otigen` binary. Deterministic genesis pre-fund (10 accounts). |
| `otigen validator` | Read-only queries over the chain-side validator registry. |
...
The counter contract ships in each language under [`pyde-net/otigen/examples/counter-{rust,go,as,c}/`](https://github.com/pyde-net/otigen/tree/main/examples). Clone any of them, run `make build && make test`, see it work.
...
- **Signed-tx commands** (`deploy` / `upgrade` / `pause` / `unpause` / `kill`) accept `--rpc-url <URL>` + `--chain-id <N>` as a paired one-shot override of the project's `[network.<name>]`. The pair is mandatory when used: a raw URL doesn't advertise a chain id, and signing against `chain_id = 0` silently bricks the FALCON signature.
