...
- **You're a chain engineer evaluating Pyde.** Skim §1 to §2 to anchor; then jump to [Your First Contract](./first-contract.md) for a working "deploy + read state" loop in 10 minutes.
...
All four are first-class. Rust, Go, and C give you a fully generated typed surface — typed storage accessors, struct-literal events, and the `() -> ()` entry dispatch, all emitted from `otigen.toml` so you write only contract logic. AssemblyScript generates the entry dispatch and typed storage accessors; its events are still hand-written against the host fns. Pick the language your team is most productive in; the size / gas deltas matter less than people-hours saved.
...
| **AssemblyScript** | ~3.0 KB | TypeScript-shaped syntax. otigen generates the entry dispatch and typed storage accessors; events are hand-written against the host fns for now. Higher per-call gas because of runtime array-bounds checks. Pick when TS familiarity outweighs the cost. |
