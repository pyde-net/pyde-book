...
- [x] JSON-RPC server (per `HOST_FN_ABI_SPEC §15.4-15.5` + chapter 17 method list) — **scaffold + two read methods shipped**: spec-compliant JSON-RPC 2.0 dispatcher over axum 0.7 (single + batch, notifications excluded from responses, five standard error codes, HTTP 204 for all-notification batches), `RpcContext` with peer_id / falcon_pubkey / listen_addrs / agent_version (`pyde-node/<CARGO_PKG_VERSION>`) / protocol_version (`pyde/1`) / `Arc<MainLoopMetrics>`, methods `pyde_getNodeInfo` + `pyde_getMetrics`, `bind_rpc_server` returns resolved `SocketAddr` synchronously so port-conflicts surface immediately, graceful shutdown via `watch::Receiver<bool>`. Wired into `pyde-node validator --rpc-listen <addr>` (optional; default disabled — no auth in v1). The full chapter-17 method set (`pyde_call`, `pyde_sendRawTransaction`, `pyde_getLogs`, etc.) lands in follow-up PRs as state + execution layers wire up. — PR [#73](https://github.com/pyde-net/engine/pull/73)
