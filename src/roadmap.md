...
- [x] `WaveCommitter` solo-validator task — **the chain commits**. tokio task that ticks at a configurable interval (default 125 ms, `--committer-tick-ms <MS>`; `0`/omit = disabled), scans the local `VertexStore` for new anchor rounds, walks the causal sub-DAG via `pyde_engine_consensus::walk_subdag`, calls `assemble_wave_commit_record` with empty β-side inputs (state_root, events, included_txs all empty for now — `StateMutator::commit_wave` populates these once β.3 ships), and persists via `consensus_store.put_wave`. `pick_solo_anchor` selects the lone vertex at a round (committee-of-one); `committed: HashSet<VertexHash>` excludes prior-wave ancestors from each new walk; `prior_anchor_round` threads through correctly. Failed rounds advance state to prevent infinite-loop. 3 new `MainLoopMetrics` counters (`wave_commit_attempts`, `waves_committed`, `wave_commit_failures`). End-to-end verified: `pyde validator --producer-tick-ms 100 --committer-tick-ms 50` produced 13 vertices + committed 12 waves in 2s with 0 failures; `pyde_getWave(0..N)` returns real `WaveCommitRecord`s with correctly-threaded anchor chain. VRF anchor selection + N/3+1 quorum vote threshold + skip rules all land when multi-validator committees come online. — PR [#97](https://github.com/pyde-net/engine/pull/97)
